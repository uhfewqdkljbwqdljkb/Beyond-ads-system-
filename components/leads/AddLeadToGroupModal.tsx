...
interface AddLeadToGroupModalProps {
  isOpen: boolean;
  onClose: () => void;
  groupId: string;
}

export const AddLeadToGroupModal: React.FC<AddLeadToGroupModalProps> = ({ isOpen, onClose, groupId }) => {
  const { user } = useAuthStore();
  const [search, setSearch] = useState('');
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const { data: leadsData, isLoading } = useLeads({ search, limit: 20 });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Leads to Group" size="md">
      <div className="space-y-4">
        ...
        <div className="h-[300px] overflow-y-auto border border-border rounded-xl bg-zinc-50/50 p-2 space-y-1 custom-scrollbar">
          {isLoading ? (
            ...
            <div className="text-center py-10 text-textMuted text-sm">No leads found matching "{search}"</div>
          ) : (
        ...
        <div className="flex justify-between items-center pt-4 border-t border-border">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={handleSubmit} disabled={selectedIds.length === 0} isLoading={addLeads.isPending}>Add Selected</Button>
          </div>
        </div>
      </div>
    </Modal>
