...
import { Card, Button, Input, Avatar, Badge } from '../ui';
import { useAuthStore } from '../../store/authStore';

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

export const BeyondAI: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  const user = useAuthStore(s => s.user);
  const [input, setInput] = useState('');
  const [messages, setMessages] = useState<{role: 'user' | 'model', text: string}[]>([
    { role: 'model', text: `Hello ${user?.firstName}. I have indexed your current pipeline and client records. How can I help you optimize your sales today?` }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const contextPrompt = `
        ...
        User Identity: ${user?.firstName} ${user?.lastName} (Role: ${user?.role})
